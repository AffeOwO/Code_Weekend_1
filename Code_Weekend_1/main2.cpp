...
struct Monster {
    int x, y, hp, gold, exp, range, attack;
};

string s; // take input
vector<Monster> monsters;
vector<pair<char, pair<int, int>>> cur_res;
int bs, bp, br, lsc, lpc, lrc, fatigue; // hero
...
int main() {
    ...
    while(fin>>s) {
        if(s=="\"base_speed\":") {
            ...
            fin >> s;
            temp_monster.range = stoi(s.substr(0, s.size()-1));
        }
        else if(s=="\"attack\":") {
            fin >> s;
            temp_monster.attack = stoi(s);
